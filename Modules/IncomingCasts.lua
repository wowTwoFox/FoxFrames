...
local function EnsureSpellBarForFrame(frame, config)
    ...
    if not container then
        ...
        container = CreateSpellBarFrame(frame, config)
...
function FF:SetupIncomingCastIndicators()
    ...
    for _, frame in ipairs(frames) do
        EnsureSpellBarForFrame(frame, config)
    end
end
...
function FF:UpdateIncomingCastIndicators()
    ...
    for _, frame in ipairs(frames) do
        local container = frame and frame.ffSpellBar
        local targetUnit = frame and frame.unit

        if not inCombat then
            container = EnsureSpellBarForFrame(frame, config) or container
        end
